...
def main():
    ...
    while True:
        ...
        if ids is not None and len(corners) > 0 and len(corners[0]) > 0:
            ...
            # Traiter toutes les détections
            total_objects, colors_found = color_detector.process_detections(frame, canny_contours, head_contours, pixel_to_cm_ratio)#(frame, canny_contours, pixel_to_cm_ratio)
            
            # Dessiner toutes les détections
            color_detector.draw_detections(frame)
    ...
    #                 text = f"Multiple needle superposed Width:{width:.1f}, Height:{height:.1f} "
    #                 cv2.putText(frame, text, text_position, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 1)
    ...
    #             if width > 0.3:  # Pour les objets assez grands pour être des aiguilles/punaises
    #                 # Extraire la ROI de la tête
    #                 head_roi = color_detector.extract_head_roi(contour, frame)
                    
    #                 if head_roi is not None:
    #                     # Détecter la couleur
    #                     color = color_detector.detect_color(head_roi)
                        
    #                     # Ajouter la couleur au texte existant
    #                     text += f" - {color}"
                        
    #                     # Optionnel : Afficher la ROI de la tête
